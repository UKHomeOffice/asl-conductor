...
module.exports = (settings, args) => {
  ...
  const fetchVersionsAtRef = async ref => {
    ...
    if (!get(response, 'data.content')) {
      throw new Error('Could not retrieve versions.yml from asl-deployments');
    }

    const content = Buffer.from(response.data.content, 'base64').toString('utf-8').trim();

    const versions = content.split(/\r?\n/).reduce((versions, line) => {
      let [serviceName, ref] = line.split(': ', 2);
      serviceName = serviceName === 'pdf-generator' ? 'html-pdf-converter' : serviceName;
      versions[serviceName] = ref;
      return versions;
    }, {});
  ...
  const getVersions = async services => {
    if (!args.ref) {
      console.log('WARNING - REF DETECTED, USING POTENTIALLY OUT OF DATE VERSIONS:');
      versions = await fetchVersionsAtRef(args.ref);
      console.log(versions);

      // // remove services that did not exist at this ref point
      // return services.filter(service => {
      //   const imageName = service.image ? service.image.split('/').pop() : service.name;
      //   return !!versions[imageName];
      // });
    }
    return services;
  };
