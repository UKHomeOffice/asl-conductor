...
const {keyBy, get, isEmpty, difference} = require('lodash');
...
module.exports = (settings, args) => {
  ...
  const normalise = service => {
    ...
    /**
     * There are two ways to control which services are running in docker:
     * * `--local <service-name>`: service-name is running locally and should not be started
     * * `--tag <tag-name>`: only services with `tag: "tag-name"`  should be started
     *
     * `--local` takes precedent over `--tag` and the service will not run.
     */

    if (args.local.includes(service.name) || (Array.isArray(args.tags) && !args.tags.includes(service.tag))) {
      service.local = true;
  ...
  const cascadeLocalToDependencies = (services) => {
    const toRunInDocker = services.filter((service) => !service.local);
    const toRunNames = toRunInDocker.map(service => service.name);
    const runningServicesWithLocalDeps = toRunInDocker
      .filter((service) => Array.isArray(service.depends_on) && difference(service.depends_on, toRunNames).length > 0)
      .map(service => service.name);

    return runningServicesWithLocalDeps.length > 0
      ? cascadeLocalToDependencies(
        services.map(
          service =>
            runningServicesWithLocalDeps.includes(service.name)
              ? {...service, local: true, host: 'host.docker.internal'}
              : service
        )
      )
      : services;
  ...
  return Promise.resolve(settings.services)
    ...
    .then(cascadeLocalToDependencies)
    ...
    .then(file => {
      const opts = ['compose', '-f', file, 'up', '--remove-orphans'];
      if (args.detach !== false) {
        opts.push('-d');
      }
      spawn('docker', opts, { stdio: 'inherit' });
    });
